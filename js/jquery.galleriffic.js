...
;(function($) {
	...
	var defaults = {
		...
		nextPageLinkText:       'Poprzedni &rsaquo;',
		prevPageLinkText:       '&lsaquo; Następny',
		enableHistory:          false,
		autoStart:              false,
		...
		$.extend(this, {
			...
			ssAdvance: function() {
				...
				if (this.settings.enableHistory)
					location.href = preHash+'#'+nextHash;
				else
					this.goto(nextIndex);
				...
				var transitionOutCallback = function() {
					...
					if (gallery.$controlsContainer) {
						gallery.$controlsContainer
							.find('div.nav-controls a.prev').attr('href', preHash+'#'+gallery.data[gallery.getPrevIndex(gallery.currentIndex)].hash).end()
							.find('div.nav-controls a.next').attr('href', preHash+'#'+gallery.data[gallery.getNextIndex(gallery.currentIndex)].hash);
					}
			...
			buildImage: function(image) {
				if (this.$imageContainer) {
					...
					// Setup image
					this.$imageContainer
						.append('<span class="image-wrapper"><a class="advance-link" rel="history" href="'+preHash+'#'+this.data[nextIndex].hash+'" title="'+image.alt+'"></a></span>')
						.find('a')
						.append(image)
						.click(function(e) {
							clickHandler(e, gallery, this);
			...
			buildPager: function(pager) {
				...
				if (this.currentPage > 0) {
					...
					pager.append('<a rel="history" href="'+preHash+'#'+this.data[prevPage].hash+'" title="'+this.settings.prevPageLinkText+'">'+this.settings.prevPageLinkText+'</a>');
				}
				...
				for (i=this.currentPage-3; i<=this.currentPage+3; i++) {
					...
					if (i == this.currentPage)
						...
						pager.append('<a rel="history" href="'+preHash+'#'+this.data[imageIndex].hash+'" title="'+pageNum+'">'+pageNum+'</a>');
					}
				}
				...
				if (nextPage < this.data.length) {
					pager.append('<a rel="history" href="'+preHash+'#'+this.data[nextPage].hash+'" title="'+this.settings.nextPageLinkText+'">'+this.settings.nextPageLinkText+'</a>');
				}
