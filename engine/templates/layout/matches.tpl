	...
	{ foreach from=$escore.matches_list.items item=match name="match"}
         <tr class="table_row">
            ...
            <td class="lright">
                ...
                {if $match->esmat_matchstatus gt 0}| <a class="rlink" href="?module=match&action=signout&id={$match->esmat_id}&login={$smarty.session.admin.essysus_login}" onclick="return confirm('Potwierdź wypisanie się klikając OK. Jeżeli zapiszesz się ponownie zostaniesz umieszcony na końcu listy!')">Wypisz się</a>{else}| <a class="glink" href="?module=match&action=signin&id={$match->esmat_id}&login={$smarty.session.admin.essysus_login}">Zapisz się</a>{/if}
                {/if}
                { if $smarty.session.admin.esurole_id eq 'ADMINISTRATOR'}
                | <a class="rlink" href="?module=match&action=deletematch&id={$match->esmat_id}">Usuń</a>
                {/if}
...
<table>
    <tr><td colspan="13" style="padding-left: 40px;">
                <div id="pages">
                        ...
                        <a class="prevnext" href="?module=match&amp;action=showmatches2&amp;from={$escore.prevfrom-1}"><< poprzednia </a> |
                        { /if }
                        ...
                        <a class="prevnext" href="?module=match&amp;action=showmatches2&amp;&amp;from={$escore.nextfrom-1}"> następna >> </a>
                        { /if }
                </div>
       </td></tr>
</table>
